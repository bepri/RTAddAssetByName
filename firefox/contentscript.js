(function () {
  ...
  for (let i of inputs) {
    if (i.type === "submit" && i.value === "+") submit_button = i;
    if (i.type === "text" && i.placeholder === "Asset #") asset_id_input = i;
  }
  ...
  asset_name_input.addEventListener("keyup", (_) => {
  ...
  async function lookup(com) {
	...
	let dummy = (new DOMParser()).parseFromString(res, "text/html");
    try {
      let table = dummy.getElementsByTagName("table")[0];
      let tbody = table.getElementsByTagName("tbody")[1];
      let td = tbody.getElementsByTagName("td")[1];
      id = parseInt(td.innerText);
    } catch (_) {} // caught later

    if (id === -1) {
      console.error("Failed to look up asset name.");
